"""smoothn — robust penalised-least-squares smoother (Garcia 2010).

Faithful Python port of Damien Garcia's MATLAB ``smoothn`` — the same robust,
DCT-based smoother PIVlab applies to its vector fields. Vendored verbatim from
the OpenPIV project (``openpiv.smoothn``) so this repo stays self-contained and
takes no hard runtime dependency on OpenPIV.

Reference: D. Garcia, "Robust smoothing of gridded data in one and higher
dimensions with missing values," Comput. Stat. Data Anal. 54 (2010) 1167-1178.

Entry point: ``smoothn(y, W=None, isrobust=True, s=None, ...) -> (z, s, exitflag, Wtot)``.
"""
